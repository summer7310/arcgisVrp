...
(function(context) {
  ...
  mixin(Indexer, /** @lends Indexer.prototype */ {
    ...
    _create_rtree: function(data, options){
      ...
      while (len--) {
        item = data[len];
        item.id = item.id || this.idField && item.attributes[this.idField];
    ...
    _create_kdtree: function(data, options){
      ...
      while (len--) {
        pt = data[len];
        id = pt.id || (this.idField && pt.attributes[this.idField]);
        if(pt.geometry){
    ...
    _insert_kdtree: function(geom, data, layerId){
      if(geom.geometry){
        ...
        data.id = data.id || (this.idField && data[this.idField]);
        geom = geom.geometry;
      }
    ...
    _insert_rtree: function(geom, data, layerId) {
      ...
      if (typeof data == 'string' || typeof data == 'number') {
        ...
        data.id = data.id || (this.idField && (data[this.idField] || data.attributes[this.idField]));
